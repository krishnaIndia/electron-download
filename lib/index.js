...
class ElectronDownloader {
  ...
  get checksumUrl () {
      ...
      return `${this.baseUrl}/${this.checksumFilename}`
  }
...
//       }
    ...
    } else if (this.opts.customFilename){
      return `${this.opts.customFilename}`
    } else {
  ...
  get url () {
    // return `${this.baseUrl}${this.middleUrl}/${this.urlSuffix}`
      return `${this.baseUrl}/${this.urlSuffix}`
  }

  get verifyChecksumNeeded () {
    return false;//semver.gte(this.version, '1.3.2')
  }
